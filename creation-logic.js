...
function storyTime(character, genderType){
  var raeStory = 'Dear Cthulhu, I have many urges and desires, but on this most sacred of nights, there is only one thing that I want. I cannot commit my request to digital media, but please look into my heart of hearts and grant my dearest wish. And I would also like a pony. Yours Truly, ' + character + '\n' + '\n' + character + 'finished ' + genderType[2] + ' letter, and with a shaking hand, clicked the Send button. ' + character + ' waited several seconds, unsure if ' + genderType[0] + ' would ever receive a response to ' + genderType[2] + ' request, but knew that in the end, if ' + genderType[2] + ' wish was granted, the results would be obvious. ' + character + ' stood, stretching, sore from sitting for so long in front of ' + genderType[2] + ' computer. ' + genderType[1] + ' glanced out the window, squinting at the ominously bright moon. It\'s a good time for a walk,' + character + ' thought.' + genderType[1] + ' put on ' + genderType[2] + ' shoes, grabbed a fresh-baked cinnamon roll, and headed out the door, into the night. It was warm and muggy, fireflies glowing in the air. ' + character + ' held out ' + genderType[2] + ' hands, catching one of the bioluminescent bugs. ' + genderType[1] + '…';

  var raeChoiceOne = 'Put the firefly in a jar.';
  var raeChoiceTwo = 'Smear the firefly along the sidewalk in an arcane symbol.';

  var carsonStory = 'With the firefly\'s fate sealed, ' + character + ' continued to stroll down the dimly lit street. Each step seemed to crash loudly in the muggy, still air, shattering the silence. Unusually quiet tonight, thought ' + character + '. ' + genderType[3] + ' eyes drifted upwards. Amidst the large rolling clouds staining the night sky, the moon seemed brighter now, teeming with what seemed to be a red glow. ' + '\n' + '\n' + 'An uneasy sensation broke ' + character + '\'s gaze. ' + genderType[3] + '  eyes darted toward the street as a sense of danger washed over ' + genderType[2] + '. All of a sudden ' + genderType[0] + '  got the sense that ' + genderType[0] + '  was being watched. ' + character + ' quickened ' + genderType[2] + '  pace as ' + genderType[0] + '  neared the trail entrance.' + '\n' + '\n' + 'Tall trees stretched above ' + genderType[2] + '  as ' + genderType[0] + '  walked upon the forest floor like giants in the darkness, when a patch of moonlight poking through the trees revealed something profound. ' + character + ' stood over a moonlit tree stump with a small dagger sitting on top of it.' + '\n' + '\n' + 'CTHULHU WAITS whispered throughout the trees surrounding ' + name + '...';
